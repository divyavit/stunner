...
type EventHandler struct {
	...
	// InitialPermissions is called while a new allocation is being created. Every peer IP
	// returned is installed as a permission on the allocation before it starts relaying, so
	// that peers may send to the relay address without waiting for the client to issue a
	// CreatePermission request. Returning nil installs no permissions, which is the standard
	// RFC 5766 behavior.
	InitialPermissions func(srcAddr, dstAddr net.Addr, protocol, userID, realm string) []net.IP
